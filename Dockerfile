...
RUN true\
    ...
    && provisioning/install-sw.sh julia-bindist 1.11.8 /opt/julia-1.11 \
    && (cd /opt/julia-1.11 && ln -s ../julia-local local) \
    && (cd /opt/julia-1.11/bin && ln -s julia julia-1.11) \
    && provisioning/install-sw.sh julia-bindist 1.12.4 /opt/julia-1.12 \
...
RUN provisioning/install-sw.sh nodejs-bindist 24.12.0 /opt/nodejs
...
RUN apt-get update && apt-get install -y \
    ...
    && wget \
        ...
        https://github.com/TurboVNC/turbovnc/releases/download/3.2.1/turbovnc_3.2.1_amd64.deb \
    && dpkg -i virtualgl_3.1.4_amd64.deb turbovnc_3.2.1_amd64.deb \
    && rm virtualgl_3.1.4_amd64.deb turbovnc_3.2.1_amd64.deb \
    && apt-get clean && rm -rf /var/lib/apt/lists/*
